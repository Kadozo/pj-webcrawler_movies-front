...
export const Movies = (props) => {
    ...
    const sendRequest = async (offset = null) => {
        ...
        const r = await (await fetch(url + `/watchable?type=movies&title=${search}`)).json();
    ...
    const infinitScroll = async () => {
        ...
        const r = await (await fetch(url + `/watchable?type=movies&title=${search}&offset=${offsetList}`)).json();
    ...
    const getImages = async () => {
        setLoading(true);
        const r = await (await fetch(url + `/watchable?type=movies&only_img=true`)).json()
        setImages(r.watchables)
        setLoading(false); 
    }

    useEffect(() => {
        if(offsetList !== 0 && !full)
            infinitScroll();
    }, [offsetList]);
